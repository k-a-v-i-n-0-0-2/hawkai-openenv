...
function processFile(filePath) {
    ...
    // 2. Replace plain `text-gray-300`, `text-[gray-400]`, etc. with `text-theme-muted`
    content = content.replace(/\btext-gray-[1-9]00\b/g, 'text-theme-muted');
    content = content.replace(/\btext-\[#([A-Fa-f0-9]{3,6})\]\b/g, (match, hex) => {
        // If it's a specifically light-ish hardcoded hex replacing, let's rely on standard utilities instead if possible.
        // Actually, let's keep hardcoded branding like text-[#FF6B00] but replace plain grays.
        return match;
    });

    // 3. For buttons or areas with `bg-theme-primary`, we need them to use the contrasting text. 
    // We will replace `text-theme-main` with `text-on-primary` on these elements.
    // A simple heuristic: if a line of code has both `bg-theme-primary` and `text-theme-main`, swap it to `text-on-primary`.
    // We'll run a few passes just in case string concatenations have them on the same line.
